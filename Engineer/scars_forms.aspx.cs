...
public partial class Engineer_scars_forms : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        ddlFailureAnalysis();
        ddlSCARStatus();
        txtVerifyCAResult.ReadOnly = true;
        string tempSCARID = Request.QueryString["scar_id"];

        if(!String.IsNullOrEmpty(tempSCARID))
        {
            Display_Approval_GridView();
            SCAR_Status_Control(tempSCARID);
            Read_Existing_Request_Records(tempSCARID);
            Read_Only_Response_Form(tempSCARID, null);
            string connect = ConfigurationManager.ConnectionStrings[DatabaseName].ConnectionString;
            using (SqlConnection conn = new SqlConnection(connect))
            {
                conn.Open();
                SqlCommand select = new SqlCommand("SELECT id, scar_stage FROM dbo.SCAR_Request WHERE id = @tempSCARID AND scar_stage = @scarStage OR scar_stage = @scarStageClosed", conn);
    ...
    protected void SCAR_Status_Control(string scar_id)
    {
        if (!IsPostBack)
        {
            string connect = ConfigurationManager.ConnectionStrings[DatabaseName].ConnectionString;
            ...
            {
                conn.Open();
                SqlCommand select = new SqlCommand("SELECT scar_status FROM dbo.SCAR_Request WHERE id = @scar_id", conn);
                ...
                while(reader.Read())
                {
                    ...
                    {
                        Read_Only_Response_Form(scar_id, "SCAR Type 2 Accepted");
                    }
                    ...
                    else if (Convert.ToString(reader["scar_status"]).Equals("SCAR Type 4 Accepted"))
                    {
                        ...
                        uploadFile.Enabled = false;
                        Read_Only_Response_Form(scar_id, null);
                    }
    ...
    protected void Read_Only_Response_Form(string scar_id, string scar_status)
    {
        string connect = ConfigurationManager.ConnectionStrings[DatabaseName].ConnectionString;
        using (SqlConnection conn = new SqlConnection(connect))
        {
            conn.Open();
            SqlCommand select = new SqlCommand("SELECT status FROM dbo.SCAR_Response WHERE scar_id = @scar_id", conn);
            select.Parameters.AddWithValue("@scar_id", scar_id);
            SqlDataReader reader = select.ExecuteReader();
            ...
            while (reader.Read())
            {
                if (Convert.ToString(reader["status"]).Equals("submit"))
                {
                    ...
                    if(!string.IsNullOrEmpty(scar_status))
                    {
                        ...
                        {
                            txtVerifyCAResult.ReadOnly = false;
                        }     
                    }
                    else
                    {
        ...
        if (!IsPostBack)
        {
            ...
            if (!String.IsNullOrEmpty(scar_id))
            {
                ...
                using (SqlConnection conn = new SqlConnection(connect))
                {
                    ...
                    {
                        conn.Open();
                        SqlCommand select = new SqlCommand(@"SELECT SCAR_Request.id, SCAR_Request.scar_status, SCAR_Request.scar_type, SCAR_Response.status 
FROM dbo.SCAR_Request INNER JOIN dbo.SCAR_Response ON SCAR_Request.id = SCAR_Response.scar_id WHERE SCAR_Request.id = @scar_id", conn);
                        select.Parameters.AddWithValue("@scar_id", scar_id);
                        SqlDataReader reader = select.ExecuteReader();
                        if(!reader.HasRows)
                        {
    ...
    protected void ddlFailureAnalysis()
    {
        if (!IsPostBack)
        {
            ...
            lstFailureAnalysis.Items.Insert(1, new ListItem("Electrical", "Electrical"));
            lstFailureAnalysis.Items.Insert(2, new ListItem("Phyisical", "Physical"));
            lstFailureAnalysis.Items.Insert(3, new ListItem("Visual", "Visual"));
        }
    }
    ...
    protected bool Check_Duplicate_Data(SCAR scar_details, int clicked_button)
    {
        ...
        while (reader.Read())
        {
            ...
            {
                if (scar_details.Car_no.CompareTo(Convert.ToString(reader["car_no"])) == 0 && scar_details.Car_revision.CompareTo(Convert.ToString(reader["car_revision"])) == 0 && scar_details.Issued_date.CompareTo(Convert.ToString(reader["issued_date"])) == 0 && Convert.ToString(reader["save_status"]) == "save" || Convert.ToString(reader["save_status"]) == "submit")
                { compare_data = false;
                } 
            }
...
@s75_verifier_email, @s76_verify_corrective_action_result_effectiveness, @defect_modes, @mor_calculated, @status, @scar_id)", con);
                ...
                finally
                {
                    ...
                    if(!uploadFile.HasFile && lstWCM.SelectedItem.Value == "0" && lstQM.SelectedItem.Value == "0")
                    {
                        Upload_Files(null, null);
            ...
            else
            {
                try
                {
                    ...
                    while (reader.Read())
                    {
                        if (temp_SCAR_ID.CompareTo(Convert.ToString(reader["scar_id"])) == 0)
                        {
                            exist_data = true;
                        }
                    }
...
mor_calculated = @mor_calculated WHERE scar_id = @scar_id", con);
                ...
                {
                    ProcessedMessage.Text = "SCAR Response has NOT been updated! Please Try Again!" +err.Message;
                    ProcessedMessage.ForeColor = System.Drawing.ColorTranslator.FromHtml("red");
                }
                finally
                {
        ...
        {
            string temp_SCAR_ID = Request.QueryString["scar_id"];
            Parse_Data_Into_Notepad(temp_SCAR_ID);
        }
    }
    ...
    protected bool Check_Duplicate_Response_Data(SCAR_Response scar_response_details, int response_clicked_button)
    {
        ...
        while (reader.Read())
        {
            if (response_clicked_button == 0)
            {
                if (temp_SCAR_ID.CompareTo(Convert.ToString(reader["scar_id"])) == 0 && Convert.ToString(reader["status"]).Equals("save") || Convert.ToString(reader["status"]).Equals("submit"))
                {
                    compare_data = false;
                }
            }
...
VALUES (@approval_status_WCM, @approval_status_QM, @comment_WCM, @comment_QM, @sent_date, @sent_time, @reject_count_WCM, @reject_count_QM, @scar_id)", con);
    ...
    protected void Parse_Data_Into_Notepad(string scar_id)
    {
        ...
        {
            conn.Open();
            SqlCommand selectRequest = new SqlCommand("SELECT scar_type, id, car_no, car_revision, car_type, pre_alert, related_car_no, related_car_ref, originator, recurrence, supplier_contact,supplier_email, issued_date, originator_dept, originator_contact, part_no, part_description, business_unit, dept_pl, commodity, defect_quantity, defect_type, non_conformity_reported, reject_reason, expected_date_close, file_name FROM dbo.SCAR_Request WHERE id = @scar_id", conn);
            selectRequest.Parameters.AddWithValue("scar_id", scar_id);
            SqlDataReader reader;
            ...
            selectResponse.Parameters.AddWithValue("scar_id", scar_id);
        ...
        try
        {
            ...
            using (System.IO.StreamWriter file = new System.IO.StreamWriter(path))
            {
                ...
                {
                    conn.Open();
                    SqlCommand update = new SqlCommand("UPDATE dbo.SCAR_Response SET status = @status WHERE scar_id = @scar_id", conn);
...
FROM dbo.Approval_8D WHERE scar_id = @scar_id", conn);
    ...
    protected void Close_SCAR_Issue(string scar_id)
    {
        string connect = ConfigurationManager.ConnectionStrings[DatabaseName].ConnectionString;
        using (SqlConnection conn = new SqlConnection(connect))
        {
            ...
            {
                conn.Open();
                SqlCommand update = new SqlCommand(@"UPDATE dbo.SCAR_Request SET scar_stage = @scar_status WHERE id = @scar_id", conn);
                update.Parameters.AddWithValue("@scar_id", scar_id);
