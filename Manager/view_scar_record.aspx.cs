...
s73_responsible_person, s74_verifier, s75_verifier_email, s76_verify_corrective_action_result_effectiveness, defect_modes, mor_calculated, 
scar_no FROM dbo.SCAR_History WHERE scar_no = @scar_no", conn);
                select.Parameters.AddWithValue("scar_no", scar_no);
                SqlDataReader reader;
                reader = select.ExecuteReader();
